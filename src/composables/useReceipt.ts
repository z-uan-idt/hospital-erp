...
export const useReceipt = () => {
  ...
  const onFetchGoodsReceivedNoteBatchByDrug = async (drugId: string | number) => {
    const params = {
      did: drugId,
    }

    const { data: response } = await fetchApi.get<IReceiptBatch[]>(
      'api/v1/warehouse/goods-received-note/batches-by-drug',
      params
    )
    if (response.success) {
  ...
  return {
    ...
    onFetchGoodsReceivedNoteBatchByDrug,
  }
}
