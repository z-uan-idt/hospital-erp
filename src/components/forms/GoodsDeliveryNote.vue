            ...
            <v-row class="px-3 pb-8">
              ...
              >
                <v-select
                  ...
                  @click.native="fetchGoodsReceivedNoteBatchByDrug(index, batch.drug)"
                >
                  <template #label>
                    <div class="d-flex align-center justify-start w-100">
                      <span>
                  ...
                  >
                    <div class="d-flex align-center justify-start w-100">
                      <span class="text-body-2 text-erp-gray-800">
                        Còn: {{ batch?.goods_received_note_batch_info?.quantity }}
                  ...
                  :rules="[
                    ...
                    (v) =>
                      batch?.goods_received_note_batch_info
                        ? v > batch?.goods_received_note_batch_info?.quantity
                          ? `Không được vượt quá ${batch?.goods_received_note_batch_info?.quantity} ${batch?.goods_received_note_batch_info?.unit?.name?.toLowerCase()}`
                          : true
                        : true,
                  ]"
                >
...
<script setup lang="ts">
  ...
  const fetchGoodsReceivedNoteBatchByDrug = async (index: number, drugId: number) => {
    formPayload.value.batches[index].loading = true
    formPayload.value.batches[index].batches = (await receiptHook.onFetchGoodsReceivedNoteBatchByDrug(drugId)).map(
      (batch) => ({
        ...batch,
        code: `#${batch.code}`,
      })
  ...
  const selectDrug = (drug: IDrug, index: number) => {
    ...
    fetchGoodsReceivedNoteBatchByDrug(index, drug.id)
  }
