  ...
  >
    <div class="d-flex align-center flex-row justify-space-between ga-4">
      <div class="d-flex align-center justify-start ga-3 w-100 w-md-auto">
        <h1 :class="['text-lg-h5 text-h6 font-weight-medium', 'text-blue-grey-darken-3']">Phiếu xuất kho</h1>
      </div>
      ...
      <div class="action-buttons d-flex align-center ga-2">
          ...
          >
            ...
            <template #text>
              <p class="text-body-2 text-erp-gray-700">
                {{
                  !warehouseSelected
                    ? 'Vui lòng chọn kho trước khi tạo phiếu xuất kho'
        ...
        <v-btn
          ...
          size="small"
          class="text-body-1"
          @click="router.push(ROUTE_PHIEU_XUAT_KHO.CREATE.path)"
        >
          <v-icon
            size="20"
            class="text-erp-gray-800"
...
<script setup lang="ts">
  ...
  const { receiptExports, ...hooks } = useReceipt()
  ...
  watch(
    ...
    async () => {
      ...
      await hooks.onFetchGoodsDeliveryNote(warehouseSelected.value)
  ...
  const onRowClick = (_: unknown, { item }) => {
    if (item.status?.value === 'DRAFT') {
      router.push(`${ROUTE_PHIEU_XUAT_KHO.CREATE.path}?id=${item.id}`)
    } else {
      router.push(ROUTE_PHIEU_XUAT_KHO.DETAIL.pathFunc(item.id))
    }
  }
